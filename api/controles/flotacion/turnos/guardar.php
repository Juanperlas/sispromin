...
if ($esEdicion) {
    if (!tienePermiso("controles.flotacion.turnos.editar")) {
        http_response_code(403);
        echo json_encode(["success" => false, "message" => "No tiene permisos para editar turnos de flotación"]);
        exit;
    }
} else {
    if (!tienePermiso("controles.flotacion.turnos.crear")) {
        http_response_code(403);
        echo json_encode(["success" => false, "message" => "No tiene permisos para crear turnos de flotación"]);
        exit;
    }
}
...
try {
    ...
    if ($esEdicion) {
        ...
        $turnoExistente = $conexion->selectOne("SELECT id FROM turnos_flotacion WHERE id = ?", [$id]);
        if (!$turnoExistente) {
        ...
        $codigoDuplicado = $conexion->selectOne(
            "SELECT id FROM turnos_flotacion WHERE codigo = ? AND id != ?",
            [$codigo, $id]
        );
        if ($codigoDuplicado) {
        ...
        $resultado = $conexion->update("turnos_flotacion", $datos, "id = ?", [$id]);
        ...
        $codigoDuplicado = $conexion->selectOne("SELECT id FROM turnos_flotacion WHERE codigo = ?", [$codigo]);
        if ($codigoDuplicado) {
            throw new Exception("Ya existe un turno con ese código");
        }
        ...
        $turnoId = $conexion->insert("turnos_flotacion", $datos);
    ...
    error_log("Error en guardar.php (turnos_flotacion): " . $e->getMessage());
}
