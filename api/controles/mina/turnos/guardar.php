...
if ($esEdicion) {
    if (!tienePermiso("controles.mina.turnos.editar")) {
        http_response_code(403);
        echo json_encode(["success" => false, "message" => "No tiene permisos para editar turnos de mina"]);
        exit;
    }
} else {
    if (!tienePermiso("controles.mina.turnos.crear")) {
        http_response_code(403);
        echo json_encode(["success" => false, "message" => "No tiene permisos para crear turnos de mina"]);
        exit;
    }
}
...
try {
    ...
    if ($esEdicion) {
        ...
        $turnoExistente = $conexion->selectOne("SELECT id FROM turnos_mina WHERE id = ?", [$id]);
        if (!$turnoExistente) {
        ...
        $codigoDuplicado = $conexion->selectOne(
            "SELECT id FROM turnos_mina WHERE codigo = ? AND id != ?",
            [$codigo, $id]
        );
        if ($codigoDuplicado) {
        ...
        $resultado = $conexion->update("turnos_mina", $datos, "id = ?", [$id]);
        ...
        $codigoDuplicado = $conexion->selectOne("SELECT id FROM turnos_mina WHERE codigo = ?", [$codigo]);
        if ($codigoDuplicado) {
            throw new Exception("Ya existe un turno con ese código");
        }
        ...
        $turnoId = $conexion->insert("turnos_mina", $datos);
    ...
    error_log("Error en guardar.php (turnos_mina): " . $e->getMessage());
}
