/**
 * Gestión de líneas de amalgamación
 * Funcionalidades para listar, crear y editar líneas de amalgamación
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initDataTable() {
    lineasTable = $("#lineas-table").DataTable({
      ...
      responsive: true,
      ajax: {
        url: getUrl("api/controles/amalgamacion/lineas/listar.php"),
        type: "POST",
        data: (d) => {
          if (d.length == -1) {
            d.length = 10000;
      ...
      columns: [
        ...
        },
        {
          ...
          render: (data) => {
            ...
            if (tienePermiso("controles.amalgamacion.lineas.editar")) {
      ...
      buttons: [
        ...
        },
        {
          ...
          customize: (doc) => {
            ...
            doc.content.unshift({
              text: "REPORTE DE LÍNEAS DE AMALGAMACIÓN",
              style: "header",
              alignment: "center",
              margin: [0, 15, 0, 15],
            });
            ...
            doc.info = {
              title: "Reporte de Líneas de Amalgamación",
              author: "SISPROMIN",
              subject: "Listado de Líneas de Amalgamación",
            };
          },
          filename:
            "Reporte_Lineas_Amalgamacion_" +
  ...
  // FUNCIONALIDAD: Función para mostrar modal con detalles de la línea
  function mostrarModalDetalles(id) {
    modalVerLinea = new bootstrap.Modal(
      document.getElementById("modal-ver-linea")
    );

    // Resetear contenido del modal
    $("#modal-ver-linea-body").html(`
    ...
    $.ajax({
      url: getUrl("api/controles/amalgamacion/lineas/obtener.php"),
      type: "GET",
      data: { id: id },
      dataType: "json",
      success: (response) => {
        if (response.success && response.data) {
          ...
          $("#modal-ver-linea-titulo").html(`
            <i class="bi bi-eye me-2"></i>Detalles de la Línea: ${data.nombre}
          `);
          ...
          $("#modal-ver-linea-body").html(contenidoModal);
  ...
  function guardarLinea() {
    ...
    $.ajax({
      url: getUrl("api/controles/amalgamacion/lineas/guardar.php"),
      type: "POST",
      data: formData,
      processData: false,
      contentType: false,
